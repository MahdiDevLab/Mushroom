...
class Mushroom {
   ...
   #colorLoop() {
      ...
      let { H, S } = this.#CL.toHslChannels(color);
      ...
      for (let i = 0; i < N.length; i++) {
         result[N[i]] = {
            H: (i * steps) % 360,
            S: H === 0 ? 100 : Math.min(100, ratios[i] * S)
         };
      }
   ...
   #palette() {
      let V = this.#V;
      let { color, colorHarmony, surfaceColor, fixedSubPalette, reverseSubPalette, contrast, customColors } = this.#C;
      let { H, S, L } = this.#CL.toHslChannels(color);
      let theme = this.currentTheme();
      let AN = ['primary', 'secondary', 'tertiary', 'quaternary'];
      let AH = this.#accentHue(colorHarmony);
      let HA = AH.map(i => i + H > 360 ? i + H - 360 : i + H)
      let sH, sS, sL;
      let ToHex = this.#CL.channels.hsl.hex;
      let result = {};
      
      if (contrast !== 'auto') L = contrast;
      if (V.color(surfaceColor)) {
         sH = this.#CL.getHue(surfaceColor);
         sS = this.#CL.getSaturation(surfaceColor);
         sL = this.#CL.getLightness(surfaceColor);
         if (contrast !== 'auto') sL = contrast;
      }
      else {
         sH = HA[AN.indexOf(surfaceColor)];
         sS = S;
         sL = L;
      }
      ...
      for (let i in D.N.accent) {
         for (let j in D.F.b)
            for (let k in D.F.a) result[D.F.a[k] + D.N.accent[i] + D.F.b[j]] = ToHex(D.H.accent[i], D.S.accent, D.L[theme].accent[j][k]);
         for (let j in D.F.f) result[D.F.f[j] + D.N.accent[i]] = ToHex(D.H.accent[i], D.S.accent, D.L[theme].accentLD[j]);
      }
      for (let i in D.F.b)
         for (let j in D.F.a) result[D.F.a[j] + D.N.error + D.F.b[i]] = ToHex(D.H.error, D.S.error, D.L[theme].accent[i][j]);
      for (let j in D.F.f) result[D.F.f[j] + D.N.error] = ToHex(D.H.error, D.S.error, D.L[theme].accentLD[j]);
      for (let i in D.N.accent)
         for (let j in D.F.a) result[D.F.a[j] + D.F.e + D.N.accent[i]] = ToHex(D.H.accent[i], D.S.accent, D.L[theme].inverse[j]);
      for (let i in D.F.a) result[D.F.a[i] + D.F.e + D.N.error] = ToHex(D.H.error, D.S.error, D.L[theme].inverse[i]);
      for (let i in D.F.a) result[D.F.a[i] + D.N.background] = ToHex(D.H.background, D.S.background, D.L[theme].background[i]);
      for (let i in D.F.c) result[D.N.surface + D.F.c[i]] = ToHex(D.H.surface, D.S.surface, D.L[theme].surface[i]);
      for (let j in D.F.a) result[D.F.a[j] + D.F.e + D.N.surface] = ToHex(D.H.surface, D.S.surface, D.L[theme].inverseSurface[j]);
      for (let i in D.F.d) result[D.N.outline + D.F.d[i]] = ToHex(D.H.outline, D.S.outline[i], D.L[theme].outline[i]);
      // custom color
      for (let i in customColors) {
         ...
         if (limit.includes(i) == false || allowed.includes(i)) {
            D.N.custom.push(i);
            D.H.custom.push(this.#CL.getHue(customColors[i]));
            D.S.custom.push(this.#CL.getSaturation(customColors[i]));
         } else {
            this.#warn('UnauthorizedCustomColor', `${i}: ${customColors[i]}`);
         }
      }
      for (let i in D.N.custom) {
         for (let j in D.F.b)
            for (let k in D.F.a) result[D.F.a[k] + D.N.custom[i] + D.F.b[j]] = ToHex(D.H.custom[i], D.S.custom[i], D.L[theme].accent[j][k]);
         for (let j in D.F.f) result[D.F.f[j] + D.N.custom[i]] = ToHex(D.H.custom[i], D.S.custom, D.L[theme].accentLD[j]);
      }
      for (let i in D.N.custom)
         for (let j in D.F.a) result[D.F.a[j] + D.F.e + D.N.custom[i]] = ToHex(D.H.custom[i], D.S.custom[i], D.L[theme].inverse[j]);
      // result
      return result;
   }
   #subPalette() {
      let V = this.#V;
      let { color, colorHarmony, surfaceColor, fixedSubPalette, reverseSubPalette, parts } = this.#C;
      let { H, S } = this.#CL.toHslChannels(color);
      let theme = this.currentTheme();
      let AN = ['primary', 'secondary', 'tertiary', 'quaternary'];
      let AH = this.#accentHue(colorHarmony);
      ...
      if (fixedSubPalette) {
         if (!reverseSubPalette) {
            
            for (let i in D.N.accent)
               for (let j in parts) result[D.N.accent[i] + '-' + parts[j]] = ToHex(D.H.accent[i], D.S.accent, parts[j]);
            for (let i in D.N.neutral)
               for (let j in parts) result[D.N.neutral[i] + '-' + parts[j]] = ToHex(D.H.neutral, D.S.neutral[i], parts[j]);
            
         } else {
            
            for (let i in D.N.accent)
               for (let j in parts) result[D.N.accent[i] + '-' + parts[j]] = ToHex(D.H.accent[i], D.S.accent, parts[parts.length - 1] - parts[j]);
            for (let i in D.N.neutral)
               for (let j in parts) result[D.N.neutral[i] + '-' + parts[j]] = ToHex(D.H.neutral, D.S.neutral[i], parts[parts.length - 1] - parts[j]);
   ...
   #CLPalette() {
      let { color } = this.#C;
      let L = this.#CL.getLightness(color);
      ...
      for (let i in loop) {
         for (let j in b)
            for (let k in a) result[a[k] + i + b[j]] = ToHex(loop[i].H, loop[i].S, l[theme].accent[j][k]);
         for (let j in c) result[c[j] + i] = ToHex(loop[i].H, loop[i].S, l[theme].accentLD[j]);
      }
      
      return result;
   }
   #CLSubPalette() {
      let { fixedSubPalette, reverseSubPalette, parts } = this.#C;
      let theme = this.currentTheme();
      let loop = this.#colorLoop();
      let ToHex = this.#CL.channels.hsl.hex;
      let result = {};
      ...
      if (fixedSubPalette) {
         if (!reverseSubPalette) {
            for (let i in loop)
               for (let j in parts) result[i + '-' + parts[j]] = ToHex(loop[i].H, loop[i].S, parts[j]);
         } else {
            for (let i in loop)
               for (let j in parts) result[i + '-' + parts[j]] = ToHex(loop[i].H, loop[i].S, parts[parts.length - 1] - parts[j]);
         }
      } else {
         if (!reverseSubPalette) {
            if (theme === 'light') {
